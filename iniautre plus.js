...
var k=6;
			...
			for (var ii=15; ii<number; ii++) {
				...
				if (nbre_caracteres("1",jj)==4) {
				   rec+="ii "+ii+" k "+k+" --> jj "+jj+" jjini "+jjini+"<br>";
				   num+=1;
				}
			}
putmessage(num+" cas sur "+number+" :<br>"+rec);
basculef();
return;
*/
	var hh=new Array(9);
...
var ntrouve=0;
for (var i=15; i<number; i++) {
	var j=decimalToBinary(i);
	if (nbre_caracteres("1",j)==4) {ntrouve+=1}
}
putmessage("Pour une chaÃ®ne de longueur "+n+" on trouve "+ntrouve+" exemples Ã  4 éléments");
// Résultat : pour (4,5,6,7,8,9) : (1,5,15,35,70,126)

}

/*
if (cquad.length==4) {// cquad complet ok; exploration hhautre
    for (var kkautre=0; kkautre<kautre; kkautre++) {// tableau contenu[hhautre]
    	var d=contenu[hhautre[kkautre]];
    	var c01=cquad.match(re);
    	var d01=d.match(re);
    	var nm=0;
    	for (var mm=0; mm<d.length; mm++) {
    		var dd=d01[mm];
    		if (cquad.split(dd).length<cquad.length) {// inclus
    		   nm+=1;
    		}
    	}// mm
    	if (nm==d.length) {//nouvelle case incluse dans cquad
    	...
    	wscen="Quadruples " + cquad + " en "+ colPlace[hh[0]]+colPlace[hh[1]]+colPlace[hh[2]]+ colPlace[hh[3]]+" Elimination ";									
    	for (var mm=0; mm<d.length; mm++) {
    		var dd=d01[mm];
    		if (cquad.split(dd).length<cquad.length) {// inclus
    		   eliminationnumero(hhautre[kkautre], dd);// elimination
    		}
    	}// mm
    }// kkautre
}// cquad
...
function PrintEcran(taille, sigle, liste, writeok) {
	if (writeok) {
		...
		putmessage("Aucun élément Ã  imprimer");
	}
	return true;
}
